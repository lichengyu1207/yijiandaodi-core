 ...
 * 初始化安全知识库
 */
export function initSecurityKnowledgeBase(config: SecurityKnowledgeBaseConfig = {}): SecurityKnowledgeBase {
  return new SecurityKnowledgeBase(config);
}

/**
 * 检测安全风险（便捷函数）
 */
export function detectSecurityRisks(content: string, config: SecurityKnowledgeBaseConfig = {}): RiskResult[] {
  const kb = new SecurityKnowledgeBase(config);
  return kb.detect(content);
}
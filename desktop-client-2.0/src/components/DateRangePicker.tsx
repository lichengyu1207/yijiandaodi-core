...
/** 时间范围筛选（P1-1 统计一期）：预设 + 自定义，desktop 与 web 统一预设映射 */
...
const DateRangePicker: React.FC<DateRangePickerProps> = ({ value, onChange }) => {
  ...
  const activePreset = PRESETS.find((p) => {
    if (p.key === 'custom') {
      return !PRESETS.some(
        (q) => q.key !== 'custom' &&
          resolvePreset(q.key).start_date === value.start_date &&
          resolvePreset(q.key).end_date === value.end_date
      )
    }
  ...
  const handlePreset = (key: string) => {
    ...
    const label = PRESETS.find((p) => p.key === key)?.label ?? '自定义'
    onChange({ label, start_date: r.start_date, end_date: r.end_date })
  }
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
      {PRESETS.map((p) => (
        <button key={p.key} onClick={() => handlePreset(p.key)} style={buttonStyle(activePreset?.key === p.key)}>
          {p.label}
        </button>
      ))}
      {isCustom && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          <input type="date" value={customStart} max={customEnd || undefined} onChange={(e) => setCustomStart(e.target.value)} style={inputStyle} />
          <span style={{ fontSize: 12, color: 'var(--text-tertiary)' }}>至</span>
          <input type="date" value={customEnd} min={customStart || undefined} onChange={(e) => setCustomEnd(e.target.value)} style={inputStyle} />
          <button onClick={() => onChange({ label: '自定义', start_date: customStart, end_date: customEnd })} style={buttonStyle(true)}>
            应用
          </button>
        </div>
      )}

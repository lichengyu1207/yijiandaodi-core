...
describe('YijianDaoDiCore', () => {
  ...
  describe('detect', () => {
    ...
    it('应该检测到 SQL 注入', () => {
      ...
      expect(risks.length).toBeGreaterThan(0)
      expect(risks.some(r => r.type === 'sqli' || r.type === 'sensitive')).toBe(true)
    })
    ...
    it('应该检测到 XSS', () => {
      ...
      expect(risks.length).toBeGreaterThan(0)
      expect(risks.some(r => r.type === 'xss' || r.type === 'sensitive')).toBe(true)
    })
    ...
    it('应该检测到密码', () => {
      ...
      expect(risks.length).toBeGreaterThan(0)
      expect(risks.some(r => r.type === 'password' || r.type === 'sensitive')).toBe(true)
    })
